...
export default function ImportPatientsModal({ isOpen, onClose, onImportSuccess }: ImportPatientsModalProps) {
    ...
    useEffect(() => {
        const handleMaximize = (e: any) => {
            if (e.detail.type === 'patients' && e.detail.operation === 'import' && e.detail.taskId === taskId) {
    ...
    const checkDuplicates = async () => {
        ...
        try {
            // Send only email/phone with indices
    ...
    const handleImport = async (skipDuplicates: boolean = false) => {
        ...
        try {
            ...
            console.log(`📥 Importing ${total} patients in ${batches.length} batches`)
                    ...
                    {step === 'preview' && (
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    Check & Import {parsedData.length} Patients
                                </button>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                ...
                                <div className="space-y-2 text-sm">
                                    <p className="text-blue-800 dark:text-blue-200">
                                        ✓ <strong>{uniqueCount}</strong> unique patients found
                                    </p>
                                    {duplicateCount > 0 && (
                                        <p className="text-orange-800 dark:text-orange-200">
                                            ⚠ <strong>{duplicateCount}</strong> potential duplicates detected (by email or phone)
                                        </p>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="text-center py-12">
                            <div className="max-w-md mx-auto">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-500 mt-4">
                                    Please wait, importing patient {importProgress.current} of {importProgress.total}...
                                </p>
                        ...
                        <div className="text-center py-12">
                            ...
                            <div className="text-gray-600 dark:text-gray-400 space-y-1">
                                <p className="text-lg font-medium text-green-600 dark:text-green-400">
                                    ✓ {importSummary.success} patients imported successfully
                                </p>
                                {importSummary.errors > 0 && (
                                    <p className="text-sm text-orange-600 dark:text-orange-400">
                                        ⚠ {importSummary.errors} patients failed (see console for details)
                                    </p>
                                )}
                            </div>
                        </div>
