...
interface VisitRow {
    patientIdentifier: string // opdNo, email, or phone
    opdNo: string
    date?: string
    diagnoses?: string
    temperament?: string
...
export default function ImportVisitsModal({ isOpen, onClose, onImportSuccess }: ImportVisitsModalProps) {
    ...
    const processData = (data: any[]) => {
        ...
        const visits: VisitRow[] = data.map((row: any) => ({
            patientIdentifier: String(row.patientIdentifier || row.opdNo || row.email || row.phone || '').trim(),
            opdNo: String(row.opdNo || '').trim(),
            date: row.date || row.Date || undefined,
            diagnoses: row.diagnoses || row.Diagnoses || undefined,
            temperament: row.temperament || row.Temperament || undefined,
            pulseDiagnosis: row.pulseDiagnosis || row.PulseDiagnosis || undefined,
            pulseDiagnosis2: row.pulseDiagnosis2 || row.PulseDiagnosis2 || undefined,
            majorComplaints: row.majorComplaints || row.MajorComplaints || undefined,
            historyReports: row.historyReports || row.HistoryReports || undefined,
            investigations: row.investigations || row.Investigations || undefined,
            provisionalDiagnosis: row.provisionalDiagnosis || row.ProvisionalDiagnosis || undefined,
            improvements: row.improvements || row.Improvements || undefined,
            specialNote: row.specialNote || row.SpecialNote || undefined,
            initials: row.initials || row.Initials || undefined,
            nextVisit: row.nextVisit || row.NextVisit || undefined,
            procedureAdopted: row.procedureAdopted || row.ProcedureAdopted || undefined,
            precautions: row.precautions || row.Precautions || undefined,
            ...
            payment: row.payment ? parseFloat(row.payment) : undefined,
            balance: row.balance ? parseFloat(row.balance) : undefined,
            helper: row.helper || row.Helper || undefined,
        }))

        // Validate required fields
        const errors: string[] = []
        visits.forEach((v, index) => {
            if (!v.patientIdentifier) {
                errors.push(`Row ${index + 1}: Missing patientIdentifier (opdNo, email, or phone)`)
            }
            if (!v.opdNo) {
                errors.push(`Row ${index + 1}: Missing opdNo`)
            }
        })
    ...
    const handleImport = async () => {
        ...
        try {
            const totalVisits = parsedData.length
            ...
            for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
                ...
                const response = await fetch('/api/visits/bulk', {
                    ...
                    body: JSON.stringify({ visits: chunk })
                })

                if (!response.ok) {
                    const error = await response.json()
                    throw new Error(`Failed to import visits: ${error.error || 'Unknown error'}`)
                }
                ...
                setImportProgress({ current: (chunkIndex + 1) * CHUNK_SIZE, total: totalVisits })
            }
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                ...
                                <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                                    <li>• Required: <strong>patientIdentifier</strong> (opdNo/email/phone), <strong>opdNo</strong></li>
                                    <li>• System will match existing patients by identifier</li>
                                    <li>• Optional: diagnoses, complaints, investigations, etc.</li>
                                    <li>• Each row = 1 visit record</li>
                                    <li>• Download template: <a href="/templates/visits_import_template.csv" download className="underline hover:text-blue-600">CSV Template</a></li>
                                </ul>
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead>
                                        <tr>
                                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Patient ID</th>
                                            ...
                                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Complaints</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {previewData.map((v, i) => (
                                            <tr key={i}>
                                                <td className="px-4 py-2 text-sm">{v.patientIdentifier}</td>
                                                <td className="px-4 py-2 text-sm">{v.opdNo}</td>
                                                <td className="px-4 py-2 text-sm">{v.provisionalDiagnosis || '-'}</td>
                                                <td className="px-4 py-2 text-sm">{v.majorComplaints || '-'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    Import {parsedData.length} Visits
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 'importing' && (
                        <div className="space-y-4">
                            <div className="text-center">
                                ...
                                <p className="text-lg font-semibold">Importing visits...</p>
                    ...
                    {step === 'success' && (
                        <div className="text-center space-y-4">
                            ...
                            <p className="text-gray-600 dark:text-gray-400">
                                Successfully imported <strong>{importSummary.success}</strong> visits
